...
generate_dummy_omics <- function(n_samples = 50, n_genes = 1000, n_metabolites = 200, n_linked = 10) {
  ...
  Z <- rnorm(n_samples) # Represents a latent factor (e.g., disease severity)
  ...
  for (i in 1:n_linked) {
    # Genes positively and negatively correlated with Z
    sign <- ifelse(i %% 2 == 0, 1, -1)
    X[, i] <- X[, i] + sign * 1.5 * Z + rnorm(n_samples, 0, 0.5) 
  }
  
  # 3. Create Metabolomics matrix (Y)
  Y <- matrix(rnorm(n_samples * n_metabolites), nrow = n_samples, ncol = n_metabolites)
  colnames(Y) <- paste0("Met_", 1:n_metabolites)
  
  # Inject signal into the first 'n_linked' metabolites
  for (i in 1:n_linked) {
    # Metabolites positively and negatively correlated with Z
    sign <- ifelse(i %% 2 == 0, -1, 1)
    Y[, i] <- Y[, i] + sign * 1.8 * Z + rnorm(n_samples, 0, 0.5)
  }
